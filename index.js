...
Usury.addUser = function(name,password,cb){
	...
	this.db('users').push({ name: name. passwordHash:this._hash(password),id:userId++});
...
Usury.auth = function(name,password,cb){
	...
	if(!user) return _simulateCallback("failed",null,cb);

	if(this._hash(password)!==user.passwordHash)  return _simulateCallback("auth failed",null,cb);

	return _simulateCallback(null,user,cb);
};
//----------------------------------------------------
Usury.createSession = function(name,password,ttl,cb){
	...
	this.auth(name,password,function(err,user){
		...
		this._simulateCallback(null,sessionId,cb);
	});
};
//----------------------------------------------------
